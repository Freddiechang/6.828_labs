...
int receive_pack(volatile char * dev_mmiobase, void * buf)
{
    ...
    if( (list_rx_desc[pack_count].status & E1000_RXD_STAT_EOP) == 0)
    {
        return -E_RX_BUSY;
    }
    if( list_rx_desc[pack_count].errors != 0)
    {
        return -E_RX_BUSY;
    }
    memmove(buf, (void *)list_rx_buffer[pack_count].buffer, list_rx_desc[pack_count].length);
    list_tx_desc[pack_count].status &= ~(E1000_RXD_STAT_DD | E1000_RXD_STAT_EOP);
